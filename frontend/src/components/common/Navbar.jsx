...
// import { LayoutDashboard, ShoppingBag, PlusCircle, LogOut, Menu, X, Package } from 'lucide-react';
...
//   const { user, logout, isCustomer, isStaffOrAdmin } = useAuth();
//   const navigate = useNavigate();
//   const [menuOpen, setMenuOpen] = useState(false);

//   const navLinks = isCustomer ? customerLinks : staffAdminLinks;
...
//   const activeClass = isCustomer
//     ? 'bg-emerald-400/10 text-emerald-400 border border-emerald-400/20'
//     : 'bg-gold-500/10 text-gold-400 border border-gold-500/20';

//   return (
//     <nav className="fixed top-0 left-0 right-0 z-50 border-b border-white/5 bg-charcoal-900/80 backdrop-blur-xl">
//       <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
//         {/* Logo */}
//         <Link to={user ? (isCustomer ? '/my-orders' : '/dashboard') : '/'} className="flex items-center gap-2.5 group">
//           <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${accentColor} flex items-center justify-center shadow-lg`}>
...
//             </span>
//             {user && (
//               <span className={`ml-2 text-xs font-mono px-1.5 py-0.5 rounded-md ${
//                 isCustomer
...
//                   pathname === to ? activeClass : 'text-gray-400 hover:text-gray-200 hover:bg-white/5'
//                 }`}
//               >
//                 <Icon size={15} />
//                 {label}
...
//                     {isCustomer ? 'Customer' : user.role === 'admin' ? 'Administrator' : 'Staff'}
//                   </p>
//                   <p className="text-sm font-medium text-cream-100">{user.name}</p>
//                 </div>
//                 <div className={`w-8 h-8 rounded-full bg-gradient-to-br ${accentColor} opacity-30 border flex items-center justify-center`}
//                   style={{ borderColor: isCustomer ? 'rgba(52,211,153,0.3)' : 'rgba(240,192,64,0.3)' }}>
//                   <span className={`text-sm font-bold ${isCustomer ? 'text-emerald-400' : 'text-gold-400'}`}>
...
//               <Link to="/customer-login" className="text-sm text-emerald-400 hover:underline px-3 py-1.5">
//                 Track Order
//               </Link>
//               <Link to="/login" className="text-sm text-gray-400 hover:text-gold-400 transition-colors px-3 py-1.5">
//                 Staff Login
//               </Link>
//             </div>
//           )}
...
// const Navbar = () => {
//   const { pathname } = useLocation();
//   const { user, logout, isCustomer, isAdmin, isStaffOrAdmin } = useAuth();
//   const navigate = useNavigate();
//   const [menuOpen, setMenuOpen] = useState(false);
...
//   const accentGradient = isCustomer ? 'from-emerald-400 to-emerald-600' : 'from-gold-400 to-gold-600';
...
//         >
//           <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${accentGradient} flex items-center justify-center shadow-lg`}>
//             <span className="text-charcoal-900 font-display font-black text-sm">C</span>
//           </div>
//           <div className="flex items-center gap-2">
//             <span className="font-display font-bold text-lg text-cream-100 group-hover:text-gold-400 transition-colors">
...
//             </span>
//             {user && (
//               <span className={`text-xs font-mono px-1.5 py-0.5 rounded-md hidden sm:inline ${
//                 isCustomer ? 'text-emerald-400 bg-emerald-400/10' :
//                 isAdmin ? 'text-gold-400 bg-gold-400/10' : 'text-blue-400 bg-blue-400/10'
//               }`}>
//                 {user.role}
//               </span>
//             )}
...
//                     {isCustomer ? 'Customer' : isAdmin ? 'Admin' : 'Staff'}
//                   </p>
//                   <p className="text-sm font-medium text-cream-100 leading-none">{user.name}</p>
//                 </div>
//                 <div className={`w-8 h-8 rounded-full flex items-center justify-center border text-sm font-bold ${
//                   isCustomer
...
//           {/* User info at top */}
//           <div className="flex items-center gap-2.5 px-4 py-3 mb-1 bg-white/3 rounded-xl">
//             <div className={`w-9 h-9 rounded-full flex items-center justify-center border text-sm font-bold flex-shrink-0 ${
//               isCustomer ? 'bg-emerald-400/15 border-emerald-400/30 text-emerald-400' :
//               isAdmin ? 'bg-gold-400/15 border-gold-400/30 text-gold-400' :
//               'bg-blue-400/15 border-blue-400/30 text-blue-400'
//             }`}>
//               {user.name[0].toUpperCase()}
//             </div>
